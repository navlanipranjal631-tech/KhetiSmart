/**
 * KhetiSmart v12 — Production-Ready Agriculture Platform
 *
 * REAL APIs INTEGRATED:
 * 1. Open-Meteo (free, no key) — live weather + 7-day forecast for all 28 states
 * 2. Agmarknet / data.gov.in — REAL mandi prices (Tier 1, official Govt. of India API)
 * 3. AI Assistant — Anthropic Claude API with full farm context + price fallback (Tier 2)
 *
 * ACCESSIBILITY UPGRADES (v12):
 * 4. GLOBAL TEXT-TO-SPEECH — floating 🔊 button on EVERY page, reads aloud in all 9 languages
 *    - Smart voice selection (picks best available voice for lang, falls back gracefully)
 *    - Chrome long-utterance fix (splits text into sentence chunks, no cutoff)
 *    - Cleans markdown/symbols/₹ before speaking — sounds natural
 *    - Language-tuned speaking rate per script (Tamil/Telugu slower than Hindi)
 *    - Animated waveform when speaking + stop button
 *    - Auto-expands tooltip on first load to teach farmers how to use it
 * 5. Voice INPUT already present (Chrome/Edge/Android)
 *
 * PRICE DATA 3-TIER FALLBACK CHAIN:
 *   Tier 1 → data.gov.in Agmarknet API (real mandi prices, state-filtered)
 *   Tier 2 → Claude AI with web_search (live web prices if Agmarknet unavailable)
 *   Tier 3 → Simulation (MSP-based drift — clearly labelled as indicative)
 */
...
const BUYERS = [
  { name:"FreshMart Retail",    type:"Retailer",   crops:"Vegetables, Fruits",   rating:4.8, dist:"12 km",  verified:true,  phone:"98201 34567", email:"purchase@freshmart.in",    whatsapp:"9820134567" },
  { name:"Hotel Taj Group",     type:"Restaurant", crops:"Herbs, Cherry Tomato", rating:4.9, dist:"45 km",  verified:true,  phone:"98765 00100", email:"produce@tajhotels.com",    whatsapp:"9876500100" },
  { name:"AgriWholesale Co.",   type:"Wholesaler", crops:"All Crops",            rating:4.5, dist:"8 km",   verified:true,  phone:"90000 12345", email:"buy@agriwholesale.co.in",  whatsapp:"9000012345" },
  { name:"BigBasket Direct",    type:"E-commerce", crops:"Vegetables, Fruits",   rating:4.7, dist:"Online", verified:true,  phone:"1800 202 1000",email:"farmers@bigbasket.com",   whatsapp:"9900112233" },
  { name:"Spice Route Exports", type:"Exporter",   crops:"Spices, Herbs",        rating:4.6, dist:"60 km",  verified:false, phone:"94430 88900", email:"export@spiceroute.in",     whatsapp:"9443088900" },
  { name:"Local Mandi Samiti",  type:"Mandi",      crops:"All Crops",            rating:4.2, dist:"3 km",   verified:true,  phone:"1800 180 1551",email:"info@mandironline.gov.in", whatsapp:"9112233445" },
];
...
const CROP_PROFITS = {
  Wheat:          { cost:28000, yield:20,  price:2300, icon:"🌾", unit:"qtl",    states:["Punjab","Haryana","Uttar Pradesh","Madhya Pradesh","Rajasthan"] },
  Rice:           { cost:32000, yield:18,  price:1800, icon:"🍚", unit:"qtl",    states:["West Bengal","Uttar Pradesh","Punjab","Andhra Pradesh","Tamil Nadu"] },
  Sugarcane:      { cost:38000, yield:350, price:325,  icon:"🎋", unit:"qtl",    states:["Uttar Pradesh","Maharashtra","Karnataka","Tamil Nadu","Gujarat"] },
  Cotton:         { cost:42000, yield:8,   price:6000, icon:"🌸", unit:"qtl",    states:["Maharashtra","Gujarat","Telangana","Andhra Pradesh","Madhya Pradesh"] },
  Soybean:        { cost:22000, yield:12,  price:3600, icon:"🫘", unit:"qtl",    states:["Madhya Pradesh","Maharashtra","Rajasthan","Karnataka"] },
  Maize:          { cost:20000, yield:25,  price:1950, icon:"🌽", unit:"qtl",    states:["Karnataka","Andhra Pradesh","Bihar","Rajasthan","Madhya Pradesh"] },
  Mustard:        { cost:18000, yield:10,  price:5400, icon:"🌻", unit:"qtl",    states:["Rajasthan","Haryana","Uttar Pradesh","Madhya Pradesh","Punjab"] },
  Groundnut:      { cost:30000, yield:14,  price:5200, icon:"🥜", unit:"qtl",    states:["Gujarat","Andhra Pradesh","Tamil Nadu","Karnataka","Rajasthan"] },
  Turmeric:       { cost:65000, yield:25,  price:8000, icon:"🟡", unit:"qtl",    states:["Telangana","Andhra Pradesh","Maharashtra","Tamil Nadu","Karnataka"] },
  Chilli:         { cost:48000, yield:15,  price:9500, icon:"🌶️", unit:"qtl",   states:["Andhra Pradesh","Karnataka","Maharashtra","Madhya Pradesh"] },
  Bajra:          { cost:14000, yield:12,  price:2300, icon:"🌾", unit:"qtl",    states:["Rajasthan","Gujarat","Haryana","Uttar Pradesh","Maharashtra"] },
  Onion:          { cost:35000, yield:100, price:900,  icon:"🧅", unit:"qtl",    states:["Maharashtra","Karnataka","Madhya Pradesh","Andhra Pradesh"] },
  Potato:         { cost:40000, yield:120, price:800,  icon:"🥔", unit:"qtl",    states:["Uttar Pradesh","West Bengal","Punjab","Bihar","Gujarat"] },
  "Cherry Tomato":{ cost:52000, yield:30,  price:2800, icon:"🍅", unit:"qtl",    states:["Karnataka","Maharashtra","Andhra Pradesh","Tamil Nadu"] },
};
...
function useLiveSensors(stateName = "Maharashtra", fieldCount = 4, customFieldNames = null) {
  const buildFields = (sName, fCount, fNames) => {
...
// Get your FREE key at: https://data.gov.in → Login → My Account → API Keys
// Replace the string below with your actual key for live data
const AGMARKNET_API_KEY = "579b464db66ec23bdd000001cdd3946e44ce4aabb209dcd7a9a4654e";
// ↑ This is a public demo key (returns limited records). Register for full access.
...
function useLivePrices(state) {
  ...
  // "loading" | "agmarknet" | "ai" | "simulation"
  ...
  useEffect(() => {
    async function fetchRealPrices() {
      ...
      try {
        ...
        console.warn("Agmarknet failed:", agErr.message, "→ trying Claude AI");
      }

      // ── TIER 2: Fall back to Claude AI with web_search ───────────────────
      try {
        const res = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            system: "You are a price data assistant. Search for today's Indian mandi/commodity prices. Return ONLY a valid JSON array, no markdown, no explanation. Format: [{crop,price,change,msp}]. Crops needed: Wheat,Rice,Onion,Tomato,Cotton,Soybean,Potato,Garlic,Turmeric,Chilli,Groundnut,Maize,Mustard,Bajra. Use ₹/quintal for price. msp is minimum support price (0 if unknown). change is % change today (positive or negative number).",
            messages: [{ role: "user", content: "Get today live Indian mandi crop prices for Wheat Rice Onion Tomato Cotton Soybean Potato Garlic Turmeric Chilli Groundnut Maize Mustard Bajra in rupees per quintal. Return only JSON array." }]
          })
        });
        ...
        const fetched = JSON.parse(jsonMatch[0]);
        if (!Array.isArray(fetched) || fetched.length < 5) throw new Error("Too few crops from Claude");
        setRates(prev => prev.map(r => {
          const match = fetched.find(f => f.crop?.toLowerCase() === r.crop?.toLowerCase());
          if (match && match.price > 100) {
            return { ...r, price: Math.round(match.price), base: Math.round(match.price),
                     change: +(match.change || 0), msp: match.msp || r.msp,
                     history: Array(10).fill(Math.round(match.price)), isReal: true, source: "ai" };
          }
          return r;
        }));
        setDataSource("ai");
        ...
        console.warn("Claude AI price fetch failed:", aiErr.message, "→ using simulation");
        // ── TIER 3: Simulation (always works) ───────────────────────────────
          ...
          <div style={{ background: isHigh?"#fef2f2":isLow?"#f0fdf4":"#fffbeb", border:`1.5px solid ${isHigh?"#fca5a5":isLow?"#86efac":"#fde68a"}`, borderRadius:12, padding:"12px 14px", marginBottom:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6 }}>
              ...
              <span style={{ fontSize:11, fontWeight:700, background:isHigh?"#fee2e2":isLow?"#dcfce7":"#fef3c7", color:isHigh?"#991b1b":isLow?"#166534":"#92400e", padding:"3px 8px", borderRadius:8 }}>
                {isHigh?"⚠️ Oversupplied":isLow?"✅ Opportunity":"🟡 Watch"}
              </span>
            </div>
            <div style={{ fontSize:12, color:isHigh?"#b91c1c":isLow?"#15803d":"#92400e", marginBottom: alts.length?8:0, lineHeight:1.5 }}>
              {isHigh ? `${selR.crop} is oversupplied — prices will likely be weak. Consider planting alternatives.` :
               isLow  ? `${selR.crop} is in short supply — strong prices expected this season. Good time to grow!` :
               `${selR.crop} supply is moderate — monitor prices before committing to a large crop.`}
            </div>
            {isHigh && alts.length > 0 && (
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:"#374151", marginBottom:5 }}>🔄 Better alternatives:</div>
...
function MicBtn({ onResult, lang, placeholder = "Tap to speak" }) {
  ...
  if (!isSupported) {
    ...
    return (
      <div style={{ display:"flex", alignItems:"center", gap:6, background:"#fffbeb", border:"1px solid #fde68a", borderRadius:10, padding:"6px 10px", fontSize:11, color:"#92400e" }}>
        🎤 Voice: open Chrome/Edge for voice input
      </div>
    );
  }
    ...
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
      ...
      {error && (
        <div style={{ fontSize:10, color:"#dc2626", textAlign:"center", maxWidth:120, lineHeight:1.3 }}>
          {error === "permission" ? "Allow mic in browser settings" :
...
function Dashboard({ t, state, lang, fields, rates, priceSource, realWeather, setPage }) {
  ...
  const sm = [
    ...
    { label:"pH Level",  v: ((liveField.r.ph - 4) / (9 - 4)) * 100,                 display:`${liveField.r.ph}`,                           c:"#22c55e" },
  ...
  const actionChips = getActionChips(d);
  ...
  const mspViolations = rates ? rates.filter(r => r.msp && r.price < r.msp) : [];
  const alerts = [
  ...
  return (
    <div className="dash-grid">
          ...
          <div>
            ...
            <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
              {[
                ...
                stateSupply && stateSupply.supply > 70 && { icon:"🔄", labelKey:"switchCrop", labelKey:"switchCropLbl", bg:"#7c2d12", tc:"#fed7aa", reason:`${stateSupply.crop} oversupplied (${stateSupply.supply}%)` },
      ...
      <div style={card}>
        ...
        <div style={{ color:"#6b7280", fontSize:13, marginBottom:10 }}>{d.condition} · 💧{effectiveHum}% · 💨{effectiveWind || d.wind}</div>
        ...
        {sm.map(s=>(
          <div key={s.label} style={{ marginBottom:8 }}>
            <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, color:"#374151", marginBottom:3 }}>
              <span>{s.label}</span><span style={{ fontWeight:600 }}>{s.display}</span>
            </div>
            <PBar v={s.v} c={s.c}/>
          </div>
        ))}
        <div style={{ marginTop:10, background:"#f0fdf4", borderRadius:8, padding:"8px 10px", fontSize:12, color:"#166534" }}>
          ✅ {liveField.st.s === "optimal" ? `Optimal for ${d.crop}` : `Status: ${liveField.st.s} — check soil monitor`}
        </div>
      </div>
      ...
      <div style={{ ...card, background:"linear-gradient(135deg,#f0fdf4,#dcfce7)" }}>
        ...
        <div style={{ textAlign:"center", padding:"10px 0" }}>
          ...
          <div style={{ color:"#16a34a", fontSize:12, marginTop:2 }}>Best for {state||"your region"}</div>
      ...
      <div className="dash-hero" style={{ background:"linear-gradient(135deg,#1e3a5f,#1e40af)", borderRadius:16, padding:"18px 22px", cursor:"pointer" }} onClick={()=>setPage&&setPage("insurance")}>
        <div style={{ display:"flex", gap:16, alignItems:"center", flexWrap:"wrap" }}>
          ...
          <div style={{ flex:1, minWidth:160 }}>
            <div style={{ color:"#93c5fd", fontSize:11, fontWeight:700, letterSpacing:1, marginBottom:4 }}>{t.newInsuranceAdv||"NEW — CROP INSURANCE ADVISOR"}</div>
          ...
          <div style={{ background:"#fff", color:"#1e40af", borderRadius:12, padding:"10px 18px", fontWeight:800, fontSize:13, flexShrink:0 }}>Check Now →</div>
        </div>
      </div>
    </div>
  );
...
function getSoilActions(r) {
  const actions = [];
  if (r.moisture < 38)       actions.push({ icon:"💧", text:"Irrigate now — soil moisture critically low", urgency:"high" });
  else if (r.moisture > 82)  actions.push({ icon:"🚫", text:"Stop irrigation — risk of waterlogging & root rot", urgency:"high" });
  if (r.ph < 5.5)            actions.push({ icon:"🧪", text:`Apply agricultural lime at 2–4 qtl/acre to raise pH from ${r.ph}`, urgency:"medium" });
  else if (r.ph > 7.8)       actions.push({ icon:"🧪", text:`Apply gypsum or sulfur at 1–2 qtl/acre to lower pH from ${r.ph}`, urgency:"medium" });
  if (r.temp > 36)           actions.push({ icon:"🌡️", text:"Apply mulching to reduce soil temperature stress", urgency:"medium" });
  if (r.humidity < 45)       actions.push({ icon:"🌫️", text:"Consider drip irrigation — low humidity increases evaporation", urgency:"medium" });
  else if (r.humidity > 90)  actions.push({ icon:"🍄", text:"High humidity — inspect for fungal disease, ensure drainage", urgency:"high" });
  if (r.nitrogen < 80)        actions.push({ icon:"🌿", text:`Apply urea at 25–30 kg/acre — N at ${r.nitrogen} kg/ha is low`, urgency: r.nitrogen < 40 ? "high" : "medium" });
  else if (r.nitrogen > 280)  actions.push({ icon:"⚠️", text:`Excess nitrogen (${r.nitrogen} kg/ha) — skip urea, risk of burn`, urgency:"medium" });
  if ((r.phosphorus||0) < 20)       actions.push({ icon:"🟣", text:`Apply DAP at 20–25 kg/acre — P at ${r.phosphorus||0} kg/ha is low`, urgency: (r.phosphorus||0) < 10 ? "high" : "medium" });
  else if ((r.phosphorus||0) > 100) actions.push({ icon:"⚠️", text:`Excess phosphorus (${r.phosphorus||0} kg/ha) — avoid DAP, locks out zinc`, urgency:"medium" });
  if (r.potassium < 100)      actions.push({ icon:"⚡", text:`Apply MOP at 15–20 kg/acre — K at ${r.potassium} kg/ha is low`, urgency: r.potassium < 60 ? "high" : "medium" });
  else if (r.potassium > 400) actions.push({ icon:"⚠️", text:`Excess potassium (${r.potassium} kg/ha) — avoid MOP, disrupts Mg/Ca`, urgency:"medium" });
  if (actions.length === 0)  actions.push({ icon:"✅", text:"All parameters optimal — no action required", urgency:"ok" });
  return actions;
}
...
function getPrecisionPlan(fields, labels) {
  ...
  return fields.map((f, i) => {
    ...
    if (r.moisture < 38) {
      ...
      steps.push({ phase:"today", icon:"💧", title: isCritical ? "Irrigate Immediately!" : "Water Your Field",
      ...
      steps.push({ phase:"today", icon:"🚫", title:"Stop Watering Now",
    ...
    // ── Soil pH ──────────────────────────────────────────────────────────────
    if (r.ph < 5.5) {
      steps.push({ phase:"this week", icon:"🧪", title:"Add Lime — Soil Too Acidic",
      ...
      steps.push({ phase:"this week", icon:"🧪", title:"Add Gypsum — Soil Too Alkaline",
    ...
    // ── Nitrogen ─────────────────────────────────────────────────────────────
    if (r.nitrogen < 40) {
      steps.push({ phase:"today", icon:"🌿", title:"Urea — URGENT! Nitrogen Very Low",
      ...
      steps.push({ phase:"2–3 days", icon:"🌿", title:"Apply Urea Fertilizer",
      ...
      steps.push({ phase:"ongoing", icon:"🛑", title:"DO NOT Add Urea — Too Much Nitrogen",
    ...
    if (p < 10) {
      steps.push({ phase:"today", icon:"🟣", title:"DAP — URGENT! Phosphorus Very Low",
      ...
      steps.push({ phase:"2–3 days", icon:"🟣", title:"Apply DAP Fertilizer",
      ...
      steps.push({ phase:"ongoing", icon:"🛑", title:"DO NOT Add DAP — Excess Phosphorus",
    ...
    // ── Potassium ────────────────────────────────────────────────────────────
    if (r.potassium < 60) {
      steps.push({ phase:"today", icon:"⚡", title:"MOP — URGENT! Potassium Very Low",
      ...
      steps.push({ phase:"this week", icon:"⚡", title:"Apply MOP Fertilizer",
      ...
      steps.push({ phase:"ongoing", icon:"🛑", title:"DO NOT Add MOP — Excess Potassium",
    ...
    // ── Temperature / fungal ─────────────────────────────────────────────────
    if (r.temp > 36) {
      steps.push({ phase:"today", icon:"🌡️", title:"Mulch to Cool Soil",
    ...
    if (r.humidity > 90) {
      steps.push({ phase:"today", icon:"🍄", title:"Spray Against Fungal Disease",
    ...
    // ── Healthy field ────────────────────────────────────────────────────────
    if (steps.length === 0) {
      steps.push({ phase:"ongoing", icon:"✅", title:"Field is Healthy — No Action Needed",
        desc:"All sensors are reading perfectly. Keep your current watering and fertilizer schedule.",
...
function SoilMonitor({ t, fields, lang }) {
  ...
  const plans      = getPrecisionPlan(fields, labels);
  ...
  const SLABEL = { optimal:"अच्छा / Good", good:"ठीक / OK", warning:"ध्यान दें / Watch", alert:"अभी करें / NOW!" };
  ...
  const phaseTag   = { today:"⚡ आज करें / DO TODAY", "2–3 days":"📅 2–3 दिन / 2–3 Days", "this week":"📅 इस हफ्ते / This Week", ongoing:"🔄 जारी रखें / Ongoing", ok:"✅ ठीक है / All Good", info:"ℹ️ जानकारी / Note" };

  return (
    <div>
      <PageH icon="🌱" title={t.nav[6]} sub="AI precision farming — exact doses, exact income. Sensor readings simulated; connect real IoT hardware via our API."/>

      {/* IoT Demo Mode Banner */}
      <div style={{ background:"linear-gradient(135deg,#fffbeb,#fef9c3)", border:"2px solid #fde68a", borderRadius:14, padding:"12px 16px", marginBottom:16, display:"flex", gap:12, alignItems:"flex-start", flexWrap:"wrap" }}>
        <div style={{ fontSize:24, flexShrink:0 }}>🛰️</div>
        <div style={{ flex:1, minWidth:200 }}>
          <div style={{ fontWeight:800, color:"#92400e", fontSize:13, marginBottom:3 }}>Demo Mode — Simulated IoT Sensors</div>
          <div style={{ color:"#78350f", fontSize:11, lineHeight:1.7 }}>
            These readings come from a <strong>real-time simulation</strong> modelled on soil science data for {" "}
            <strong>your selected state</strong>. Values drift every 6 seconds to simulate LoRaWAN sensor telemetry.
            In production, this panel connects to <strong>actual soil probes</strong> via the KhetiSmart IoT API (MQTT/HTTP).
          </div>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginTop:8 }}>
            {[["🌡️","Temperature","DS18B20 probe"],["💧","Moisture","Capacitive sensor"],["⚗️","pH","Electrochemical sensor"],["🧪","NPK","Optical spectrometer"]].map(([icon,name,hw])=>(
              <div key={name} style={{ background:"rgba(255,255,255,0.7)", borderRadius:8, padding:"5px 9px", fontSize:10, color:"#92400e", border:"1px solid #fde68a" }}>
                <span>{icon}</span> <strong>{name}</strong> <span style={{color:"#a16207"}}>({hw})</span>
              </div>
            ))}
          </div>
        </div>
        <div style={{ background:"#14532d", color:"#fff", borderRadius:10, padding:"6px 12px", fontSize:10, fontWeight:800, flexShrink:0, alignSelf:"center", whiteSpace:"nowrap" }}>
          🟢 DEMO LIVE
        </div>
      </div>
      ...
      {editingNames ? (
        <div style={{background:"#fff",border:"2px solid #16a34a",borderRadius:14,padding:"16px 18px",marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>setEditingNames(false)} style={{flex:1,padding:"10px",borderRadius:10,border:"1.5px solid #e5e7eb",background:"#f9fafb",color:"#374151",fontWeight:600,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>Cancel</button>
        ...
        <div className="sfade">
          ...
          <div style={{ borderRadius:16, padding:"14px 18px", marginBottom:16,
            ...
            <div style={{ fontWeight:900, fontSize:17, color: alertCnt>0?"#dc2626":warnCnt>0?"#d97706":"#166534", marginBottom:4 }}>
              <span className="ldot"/>
              {alertCnt>0 ? `🚨 ${alertCnt} खेत में तुरंत काम करें! / Field${alertCnt>1?"s":""} Need Help NOW!`
                : warnCnt>0 ? `⚠️ ${warnCnt} खेत पर ध्यान दें / Watch ${warnCnt} Field${warnCnt>1?"s":""}`
                : "✅ सभी खेत ठीक हैं / All Fields Healthy"}
            </div>
            {alertCnt>0 && <div style={{ fontSize:12, color:"#6b7280" }}>नीचे लाल खेत पर टैप करें → AI Plan देखें / Tap red field → see AI Plan</div>}
            <div style={{ display:"flex", gap:10, marginTop:10 }}>
              <div style={{ background:"rgba(255,255,255,0.75)", borderRadius:10, padding:"6px 14px", textAlign:"center" }}>
                ...
                <div style={{ fontSize:10, color:"#6b7280" }}>अच्छे खेत / Healthy</div>
              </div>
              <div style={{ background:"rgba(255,255,255,0.75)", borderRadius:10, padding:"6px 14px", textAlign:"center" }}>
                <div style={{ fontSize:18 }}>💧</div>
                <div style={{ fontSize:15, fontWeight:900, color:"#111827" }}>{(fields.reduce((s,f)=>s+f.r.moisture,0)/fields.length).toFixed(0)}%</div>
                <div style={{ fontSize:10, color:"#6b7280" }}>औसत नमी / Avg Water</div>
              </div>
            </div>
          </div>
            ...
            {[...fields].sort((a,b)=>(SEV[a.st.s]??3)-(SEV[b.st.s]??3)).map((f)=>{
              ...
              return (
                <div key={f.id} className={isUrg?"urg":""} onClick={()=>{setSelIdx(fi); if(isUrg||isWarn) setTab("precision");}}
                  ...
                  {[
                    { icon:"💧", label:"पानी / Water",    val:+f.r.moisture,       lo:38, hi:82,  max:100, unit:"%" },
                    { icon:"🌿", label:"नाइट्रोजन / N",   val:+f.r.nitrogen,       lo:80, hi:280, max:300, unit:" kg" },
                    { icon:"🟣", label:"फास्फोरस / P",   val:+(f.r.phosphorus||0), lo:20, hi:100, max:120, unit:" kg" },
                    { icon:"⚡", label:"पोटेशियम / K",   val:+f.r.potassium,       lo:100,hi:400, max:450, unit:" kg" },
                  ].map(m=>{
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginTop:6 }}>
                    {[
                      ...
                      { icon:"🌡️", label:"तापमान / Temp", val:`${(+f.r.temp).toFixed(1)}°`, ok:isOk("temp",f.r.temp) },
                    ].map(m=>(
                  ...
                  {isUrg && (
                    <div style={{ marginTop:10, background:"#dc2626", borderRadius:10, padding:"8px 12px", textAlign:"center" }}>
                      <div style={{ fontSize:12, color:"#fff", fontWeight:800 }}>🤖 AI Plan देखें → / See AI Fix Plan →</div>
                    </div>
                  )}
                </div>
              );
      ...
      {tab==="precision" && (
        <div className="sfade">
          ...
          <div style={{ background:"linear-gradient(135deg,#14532d,#166534)", borderRadius:18, padding:"18px 20px", marginBottom:18, display:"flex", gap:14, alignItems:"center" }}>
            ...
            <div>
              <div style={{ fontWeight:900, fontSize:16, color:"#fff", marginBottom:4 }}>AI खेती सलाह / AI Precision Farming</div>
              <div style={{ fontSize:12, color:"#86efac", lineHeight:1.5 }}>IoT sensors → सटीक खाद की मात्रा → अधिक कमाई<br/>Exact fertilizer doses · Step-by-step · Expected ₹ gain</div>
            </div>
          </div>
            ...
            <div key={plan.fieldIdx} style={{ marginBottom:20, borderRadius:18, overflow:"hidden", border:`3px solid ${plan.statusColor}`, background:"#fff", boxShadow:"0 2px 12px rgba(0,0,0,0.07)" }}>
              ...
              <div style={{ background: SBKG[plan.status]||"#f9fafb", padding:"16px 18px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                ...
                <button onClick={()=>speakPlan(plan)} title="Listen in your language" style={{
                  ...
                  <span style={{ fontSize:9, color:"#86efac", fontWeight:700 }}>सुनें / Listen</span>
                </button>
              </div>
              ...
              {plan.steps.some(s=>s.upliftPct>0) && (
                <div style={{ background:"linear-gradient(90deg,#ecfdf5,#d1fae5)", padding:"14px 18px", borderBottom:"1px solid #a7f3d0" }}>
                  <div style={{ display:"grid", gridTemplateColumns:"1fr auto 1fr", gap:8, alignItems:"center", textAlign:"center" }}>
                    <div>
                      <div style={{ fontSize:11, color:"#065f46", marginBottom:3 }}>आप खर्च करें / You Spend</div>
                    ...
                    <div>
                      <div style={{ fontSize:11, color:"#065f46", marginBottom:3 }}>आप कमाएं / You Earn Extra</div>
                  ...
                  <div style={{ textAlign:"center", marginTop:10, background:"rgba(255,255,255,0.7)", borderRadius:10, padding:"8px" }}>
                    <div style={{ fontSize:14, fontWeight:900, color:"#065f46" }}>
                      💰 इस मौसम में {plan.totalUplift}% अधिक फसल · +₹{plan.netGain.toLocaleString()} शुद्ध लाभ
                    </div>
              ...
              {[
                { bucket: plan.todaySteps,  heading:"⚡ आज करें — Do TODAY", color:"#dc2626", bg:"#fef2f2" },
                { bucket: plan.weekSteps,   heading:"📅 2–3 दिन में — This Week",  color:"#d97706", bg:"#fffbeb" },
                { bucket: plan.monthSteps,  heading:"🔄 जानकारी — Notes",          color:"#6b7280", bg:"#f9fafb" },
                      ...
                      <div key={si} style={{ borderRadius:14, overflow:"hidden", border:`2px solid ${phaseBorder[step.urgency]||phaseBorder[step.phase]||"#e5e7eb"}`, background:phaseBg[step.urgency]||phaseBg[step.phase]||"#fff" }}>
                        ...
                        {step.dose && step.dose !== "—" && (
                          <div style={{ margin:"0 14px 10px", background:"#1e3a5f", borderRadius:10, padding:"10px 14px", display:"flex", alignItems:"center", gap:10 }}>
                            ...
                            <div>
                              <div style={{ fontSize:10, color:"#93c5fd", fontWeight:600, marginBottom:2 }}>मात्रा / DOSE PER ACRE</div>
                        ...
                        {step.upliftPct > 0 && (
                          <div style={{ padding:"7px 14px", background:"rgba(22,163,74,0.07)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                            <span style={{ fontSize:11, color:"#166534", fontWeight:600 }}>📈 अपेक्षित वृद्धि / Expected boost</span>
          ...
          <div style={{ background:"linear-gradient(135deg,#1e3a5f,#1e40af)", borderRadius:18, padding:"18px", marginBottom:16 }}>
            <div style={{ fontWeight:900, fontSize:14, color:"#fff", marginBottom:12 }}>🏪 खाद कहाँ से लें? / Where to buy fertilizers?</div>
...
function MarketInsights({ t, rates, state, priceStatus, mandiMeta }) {
  ...
  return (
    <div>
      <PageH icon="📊" title={t.nav[8]} sub="Live mandi rates · MSP-based signals · Prices update every 4 seconds"/>
        ...
        <div>
          ...
          <div style={card}>
            <div style={{ display:"grid", gridTemplateColumns:"1.4fr 1fr 0.8fr 80px 0.8fr", gap:6, marginBottom:6, padding:"0 4px" }}>
              {["Crop","Price/qtl","Change","Trend","Signal"].map(h=>(
                <div key={h} style={{ fontSize:10, fontWeight:700, color:"#9ca3af", textTransform:"uppercase", letterSpacing:"0.5px" }}>{h}</div>
              ))}
            </div>
            {sorted.length === 0 ? (
              <div style={{ textAlign:"center", padding:20, color:"#9ca3af", fontSize:13 }}>{t.noMatchFilter||"No crops match this filter"}</div>
            ) : sorted.map(r => {
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={card}>
            ...
            {opportunities.length === 0 ? (
              <div style={{ fontSize:12, color:"#9ca3af" }}>{t.noStrongOpps||"No strong opportunities right now"}</div>
            ) : opportunities.map(r => {
              const s = SUPPLY_REGIONS.find(sr => sr.crop === r.crop);
              return (
          ...
          <div style={card}>
            ...
            {avoidList.length === 0 ? (
              <div style={{ fontSize:12, color:"#9ca3af" }}>{t.noRiskCrops||"No major risk crops currently"}</div>
            ) : avoidList.map(r => {
              const s = SUPPLY_REGIONS.find(sr => sr.crop === r.crop);
              return (
...
function ProfitEstimator({ t, rates, state }) {
  // Default to a state-relevant crop
  const getDefaultCrop = () => {
    if (!state) return "Wheat";
    const entry = Object.entries(CROP_PROFITS).find(([,v]) => v.states?.includes(state));
    return entry ? entry[0] : "Wheat";
  };

  const [sel, setSel]     = useState(getDefaultCrop);
  const [acres, setAcres] = useState(2);
  const [costAdj, setCostAdj] = useState(0); // cost adjustment % slider (-30 to +50)

  // Sync to live market price if available
  const liveRate = rates?.find(r => r.crop === sel);
  const c = CROP_PROFITS[sel];
  const livePrice  = liveRate ? liveRate.price : c.price;
  const adjCost    = Math.round(c.cost * (1 + costAdj / 100));

  const rev        = c.yield * livePrice * acres;
  const totalCost  = adjCost * acres;
  const profit     = rev - totalCost;
  const roi        = totalCost > 0 ? ((profit / totalCost) * 100).toFixed(0) : 0;
  const breakEvenPrice = totalCost > 0 ? Math.ceil(totalCost / (c.yield * acres)) : 0;
  const breakEvenQtl   = livePrice > 0 ? Math.ceil(totalCost / livePrice) : 0;

  const isLocalCrop = c.states?.includes(state);

  // Bar chart data — all crops sorted by net profit at current acreage
  const safeRates = rates || [];
  const barData = Object.entries(CROP_PROFITS).map(([name, cr]) => {
  ...
  return (
    <div>
      <PageH icon="💰" title={t.nav[7]} sub="How much will I earn? — uses today's live mandi prices"/>

      <div className="mob-full" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        {/* Left: inputs */}
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:14, marginBottom:14, ...titleFont, color:"#374151" }}>{t.farmDetails}</div>

            {/* Crop selector */}
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:12, fontWeight:600, color:"#374151", marginBottom:6 }}>{t.selectCropLbl||"Select Crop"}</div>
              <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                {Object.keys(CROP_PROFITS).map(cr => {
                  const isLoc = CROP_PROFITS[cr].states?.includes(state);
                  return (
                    <button key={cr} onClick={()=>setSel(cr)} style={{ padding:"5px 11px", borderRadius:16,
                      border: sel===cr ? "2px solid #16a34a" : "1px solid #e5e7eb",
                      background: sel===cr ? "#f0fdf4" : isLoc ? "#fafff5" : "white",
                      color: sel===cr ? "#16a34a" : "#374151", cursor:"pointer", fontSize:11,
                      fontWeight: sel===cr ? 700 : 400, fontFamily:"inherit", position:"relative" }}>
                      {CROP_PROFITS[cr].icon} {cr}
                      {isLoc && sel!==cr && <span style={{ position:"absolute", top:-3, right:-3, width:6, height:6, borderRadius:"50%", background:"#16a34a" }}/>}
                    </button>
                  );
                })}
              </div>
              {isLocalCrop && <div style={{ fontSize:10, color:"#16a34a", marginTop:5 }}>✓ Commonly grown in {state}</div>}
            </div>

            {/* Land slider */}
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:12, fontWeight:600, color:"#374151", marginBottom:4 }}>Land Size: <span style={{ color:"#16a34a" }}>{acres} acres</span></div>
              <input type="range" min={0.5} max={20} step={0.5} value={acres} onChange={e=>setAcres(+e.target.value)} style={{ width:"100%", accentColor:"#16a34a" }}/>
            ...
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:12, fontWeight:600, color:"#374151", marginBottom:4 }}>
                Input Cost Adjustment: <span style={{ color: costAdj>0?"#dc2626":costAdj<0?"#16a34a":"#374151" }}>{costAdj>0?"+":""}{costAdj}%</span>
              </div>
              <input type="range" min={-30} max={50} step={5} value={costAdj} onChange={e=>setCostAdj(+e.target.value)} style={{ width:"100%", accentColor:"#d97706" }}/>
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:"#9ca3af" }}><span>-30% (low inputs)</span><span>+50% (high inputs)</span></div>
            </div>

            {/* Assumptions card */}
            <div style={{ background:"#f9fafb", borderRadius:10, padding:"12px" }}>
              <div style={{ fontSize:11, fontWeight:700, color:"#374151", marginBottom:6 }}>{t.assumptionsLbl||"Assumptions"}</div>
              {[
                ["Cost/acre", `₹${adjCost.toLocaleString()} ${costAdj!==0?`(base ₹${c.cost.toLocaleString()})`:""}`],
                ["Yield/acre", `${c.yield} ${c.unit}`],
                ["Market price", liveRate
                  ? <span>₹{livePrice.toLocaleString()}<span style={{ fontSize:9, color:"#16a34a", marginLeft:4 }}>▶ LIVE</span></span>
                  : `₹${livePrice.toLocaleString()}`],
              ].map(([l,v])=>(
                <div key={l} style={{ display:"flex", justifyContent:"space-between", padding:"5px 0", borderBottom:"1px solid #e5e7eb", fontSize:12 }}>
          ...
          <div style={{ ...card, borderTop:"3px solid #f59e0b" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              <div style={{ background:"#fffbeb", borderRadius:10, padding:"10px 12px", textAlign:"center" }}>
                <div style={{ fontSize:11, color:"#92400e", marginBottom:2 }}>{t.sellAtLeastLbl||"Sell at least this price"}</div>
                <div style={{ fontSize:18, fontWeight:800, color:"#78350f" }}>₹{breakEvenPrice.toLocaleString()}</div>
                <div style={{ fontSize:9, color:"#9ca3af" }}>per {c.unit} to break even</div>
                <div style={{ fontSize:10, color: livePrice>breakEvenPrice?"#16a34a":"#dc2626", fontWeight:600, marginTop:3 }}>
                  {livePrice>breakEvenPrice ? `✓ Market ₹${(livePrice-breakEvenPrice).toLocaleString()} above` : `⚠️ Market ₹${(breakEvenPrice-livePrice).toLocaleString()} below`}
                </div>
              </div>
              <div style={{ background:"#fffbeb", borderRadius:10, padding:"10px 12px", textAlign:"center" }}>
                <div style={{ fontSize:11, color:"#92400e", marginBottom:2 }}>{t.minQtyLabel||"Min. Quantity"}</div>
                <div style={{ fontSize:18, fontWeight:800, color:"#78350f" }}>{breakEvenQtl}</div>
                <div style={{ fontSize:9, color:"#9ca3af" }}>{c.unit} to cover all costs</div>
                <div style={{ fontSize:10, color:"#6b7280", marginTop:3 }}>
                  of {(c.yield * acres)} {c.unit} expected
                </div>
              </div>
            </div>
            <div style={{ marginTop:10 }}>
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:"#6b7280", marginBottom:3 }}>
                <span>{t.breakevenLabel||"Break-even progress"}</span>
                <span style={{ fontWeight:700, color: livePrice>breakEvenPrice?"#16a34a":"#dc2626" }}>
                  {livePrice>breakEvenPrice ? `${Math.min(100,Math.round((livePrice/breakEvenPrice)*100))}% covered` : "Not covered"}
                </span>
              </div>
              <PBar v={Math.min(100,(livePrice/breakEvenPrice)*100)} c={livePrice>breakEvenPrice?"#16a34a":"#dc2626"}/>
            </div>
          </div>
        </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {/* Profit summary */}
          <div style={{ ...card, background: profit>=0 ? "linear-gradient(135deg,#f0fdf4,#dcfce7)" : "linear-gradient(135deg,#fef2f2,#fee2e2)" }}>
            <div style={{ ...titleFont, fontWeight:700, fontSize:14, color: profit>=0?"#14532d":"#991b1b", marginBottom:14 }}>
              {t.profitSummary} — {acres} acres
            </div>
            {[
              [t.revenue,   `₹${rev.toLocaleString()}`,        "#16a34a", false],
              [t.cost,      `₹${totalCost.toLocaleString()}`,   "#dc2626", false],
              [t.netProfit, `₹${profit.toLocaleString()}`,      profit>=0?"#14532d":"#dc2626", true],
              [t.roi,       `${roi}%`,                          profit>=0?"#2563eb":"#dc2626", true],
            ].map(([l,v,col,big],i)=>(
              <div key={l} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:i<3?`1px solid ${profit>=0?"#bbf7d0":"#fca5a5"}`:"none" }}>
                <span style={{ color:"#374151", fontWeight:big?700:400 }}>{l}</span>
                <span style={{ fontWeight:800, fontSize:big?(i===2?26:20):16, color:col }}>{v}</span>
              </div>
            ))}
            {liveRate && (
              <div style={{ marginTop:8, fontSize:10, color:"#16a34a", display:"flex", alignItems:"center", gap:4 }}>
                <span style={{ width:6, height:6, borderRadius:"50%", background:"#16a34a", display:"inline-block" }}/>
                Using live price ₹{livePrice.toLocaleString()}/qtl — updates every 4s
              </div>
            )}
          </div>
          ...
          {/* Visual bar chart */}
          <div style={card}>
            <div style={{ ...titleFont, fontWeight:700, fontSize:13, color:"#374151", marginBottom:12 }}>
              {t.compareAll} <span style={{ fontSize:11, color:"#9ca3af", fontWeight:400 }}>({acres} acres · click to select)</span>
            </div>
            {barData.map(b => {
              ...
              return (
                <div key={b.name} onClick={()=>setSel(b.name)}
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, marginBottom:3 }}>
                    ...
                    <span style={{ fontWeight:700, color: isNeg?"#dc2626":b.profit>50000?"#16a34a":"#d97706", fontSize:11 }}>
                      {isNeg?"-":""}₹{Math.abs(Math.round(b.profit)).toLocaleString()}
                    </span>
                  </div>
                  <div style={{ background:"#f3f4f6", borderRadius:4, height:7, overflow:"hidden" }}>
                    <div style={{ width:`${barW}%`, background: isNeg?"#ef4444":b.profit>50000?"#16a34a":"#f59e0b", height:"100%", borderRadius:4, transition:"width 0.4s" }}/>
                  </div>
                </div>
              );
            })}
...
function Weather({ t, state, liveWeather, weatherLoading }) {
  ...
  return (
    <div>
      <PageH icon="🌤️" title={t.nav[1]} sub={`7-day forecast for ${state||"your region"} · Farming advisories`}/>
      {isRealWeather ? (
        <div style={{ marginBottom:14, background:"#d1fae5", border:"1.5px solid #86efac", borderRadius:10, padding:"8px 14px", display:"flex", alignItems:"center", gap:8, fontSize:12, color:"#166534", fontWeight:600 }}>
          🟢 <strong>Live weather from Open-Meteo API</strong> — Real-time data for {state} · {effectiveIcon} {effectiveTemp}°C · 💧{effectiveHum}% humidity
        </div>
      ) : weatherLoading ? (
        <div style={{ marginBottom:14, background:"#fffbeb", border:"1.5px solid #fde68a", borderRadius:10, padding:"8px 14px", display:"flex", alignItems:"center", gap:8, fontSize:12, color:"#92400e", fontWeight:600 }}>
          ⏳ Fetching live weather for {state}…
        </div>
      ) : null}
      <div className="mob-full" style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:16 }}>
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          {/* Best farming days */}
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:13, color:"#374151", marginBottom:10 }}>🗓️ Best Farming Days This Week</div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
              {[
                ...
                <div key={labelKey} style={{ background:bg, borderRadius:10, padding:"10px 12px", textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:16, fontWeight:800, color:c }}>{day ? day.d : "No ideal day"}</div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ ...card, background:"linear-gradient(135deg,#f0f9ff,#e0f2fe)" }}>
            ...
            {[
              ["🌡️ High / Low",  `${days[0].h}° / ${days[0].l}°C`],
              ["💧 Humidity",     `${d.hum}%`],
              ["☔ Rain Chance",  `${days[0].rain}%`],
              ["💨 Wind",         `${days[0].wind} km/h`],
              ["🌤️ Condition",   days[0].i + " " + (days[0].rain>60?"Rainy":days[0].rain>30?"Cloudy":days[0].rain>15?"Partly Cloudy":"Clear")],
              ["📍 Region",       state || "—"],
            ].map(([l,v])=>(
...
function HighValueCrops({ t, state }) {
  ...
  return (
    <div>
      <PageH icon="💎" title={t.nav[5]} sub="Premium crops with high profit potential — click any crop for full details"/>
      {state && (
        <div style={{ background:"#eff6ff", border:"1.5px solid #bfdbfe", borderRadius:12, padding:"10px 16px", marginBottom:14, display:"flex", gap:12, alignItems:"center" }}>
          <span style={{ fontSize:22 }}>📍</span>
          <div>
            <div style={{ fontWeight:700, fontSize:13, color:"#1e40af" }}>Showing crops for {state}</div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14, flexWrap:"wrap", gap:10 }}>
        <div style={{ display:"flex", gap:6 }}>
          {[
            { k:"all",     l:"All Crops" },
            { k:"local",   l:`📍 Suit ${state||"My State"}` },
            { k:"lowrisk", l:"🟢 Low Risk" },
          ].map(({k,l})=>(
            <button key={k} onClick={()=>setFilter(k)}
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:12, color:"#374151" }}>
          <span style={{ fontWeight:600 }}>{t.acreageLabel||"Acreage"}:</span>
      ...
      <div style={{ display:"grid", gridTemplateColumns: selCrop ? "1fr 340px" : "1fr", gap:16 }}>
        <div>
          {/* Profit comparison bar chart */}
          <div style={{ ...card, marginBottom:14 }}>
            <div style={{ fontWeight:700, fontSize:13, color:"#374151", marginBottom:10 }}>
              💰 Profit Comparison — {acres} acre{acres!==1?"s":""}
              ...
              return (
                <div key={c.name} onClick={()=>setSel(isSel?null:c.name)}
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, marginBottom:4 }}>
                    <span style={{ fontWeight:isSel?700:500, color:"#374151" }}>
                      {c.emoji} {c.name}
                      {c.isLocal && <span style={{ fontSize:9, color:"#16a34a", marginLeft:5 }}>📍 Your region</span>}
                      {!c.isLocal && state && <span style={{ fontSize:9, color:"#dc2626", marginLeft:5, fontWeight:700 }}>⚠️ Not for {state}</span>}
                    </span>
                    <div style={{ textAlign:"right" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={{ ...card, borderTop:`4px solid ${selCrop.risk==="Low"?"#16a34a":selCrop.risk==="Medium"?"#f59e0b":"#ef4444"}` }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12 }}>
                {[
                  { labelKey:"estProfit", val:`₹${selCrop.totalProfit>=100?(selCrop.totalProfit/100).toFixed(1)+"L":selCrop.totalProfit+"K"}`, sub:`for ${acres} acres`, c:"#16a34a" },
                  { labelKey:"perAcre",    val:selCrop.profitLabel,      sub:"avg net profit",    c:"#2563eb" },
                  { labelKey:"riskLevel",  val:selCrop.risk,             sub:"investment risk",   c:selCrop.risk==="Low"?"#16a34a":selCrop.risk==="Medium"?"#d97706":"#dc2626" },
              ...
              <div style={{ marginBottom:10 }}>
                <div style={{ fontSize:11, fontWeight:700, color:"#374151", marginBottom:5 }}>📍 Best States</div>
                ...
                {!selCrop.isLocal && state && (
                  <div style={{ fontSize:10, color:"#dc2626", marginTop:5, fontWeight:600 }}>
                    ⚠️ Not suitable for {state} — check before planting
                  </div>
                )}
              </div>
              ...
              <div style={{ background: selCrop.demand==="Very High"?"#dbeafe":selCrop.demand==="Rising"?"#d1fae5":"#fef3c7",
                ...
                fontWeight:600 }}>
                📈 Market Demand: {selCrop.demand}
              </div>

              {/* Cultivation tip */}
              <div style={{ background:"#f0fdf4", borderRadius:8, padding:"9px 11px", marginBottom:10, fontSize:11, color:"#166534", lineHeight:1.5 }}>
                🌱 <strong>{t.cultivationTipLbl||"Cultivation tip"}:</strong> {selCrop.tip}
              </div>

              {/* Buyer channels */}
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:"#374151", marginBottom:5 }}>🤝 Where to Sell</div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
...
8. End every answer with ONE specific next action the farmer should take today`;
  ...
  const getOfflineAnswer = (q) => {
    ...
    const PEST_GUIDE = {
      ...
      default:`For ${stateCrop}: use IPM — neem oil 5ml/L for sucking pests, Trichoderma seed treatment, crop rotation. Consult KVK (1800-180-1551) for diagnosis.`,
    };
    const FERT_GUIDE = {
      ...
      default:`For ${stateCrop} on ${stateSoil}: apply basal NPK (DAP + MOP) at sowing, split urea top-dressing. Get free soil test at KVK to get exact dose.`,
    };
    const IRRIGATE_GUIDE = {
      Wheat:`Critical stages: CRI (21 days), tillering (45 days), jointing (65 days), heading (85 days), grain fill (105 days). 5–6 irrigations total. Each 5–6 cm depth.`,
      ...
      default:`For ${stateCrop}: irrigate at critical growth stages. Check soil at 6-inch depth — water when dry. Drip irrigation saves 40% water.`,
    };
    const alertAns = alertField
      ? `⚠️ ${alertField.name} needs attention: ${alertField.st?.issues?.join(", ")||"stress detected"}. ${alertField.r?.moisture < 38 ? `Moisture at ${alertField.r.moisture}% — irrigate immediately with 5cm water.` : ""} ${alertField.r?.ph < 5.5 ? `pH at ${alertField.r.ph} — apply agricultural lime 200kg/acre.` : ""} ${alertField.r?.nitrogen < 80 ? `Nitrogen at ${alertField.r.nitrogen}kg/ha — apply Urea 20kg/acre top-dressing.` : ""}`
      : null;

    if ((ql.includes("sensor") || ql.includes("field") || ql.includes("iot") || ql.includes("alert")) && alertAns) return alertAns;
    if (ql.includes("disease") || ql.includes("pest") || ql.includes("spray") || ql.includes("insect") || ql.includes("fungus") || ql.includes("blight") || ql.includes("worm") || ql.includes("bug")) return `🐛 ${stateCrop} pest & disease guide for ${state}:\n\n${PEST_GUIDE[stateCrop]||PEST_GUIDE.default}\n\n📞 For field diagnosis: KVK helpline 1800-180-1551 (free)`;
    if (ql.includes("fertiliz") || ql.includes("urea") || ql.includes("dap") || ql.includes("nitrogen") || ql.includes("nutrient") || ql.includes("khad") || ql.includes("khatu")) return `🌿 Fertilizer schedule for ${stateCrop} in ${state}:\n\n${FERT_GUIDE[stateCrop]||FERT_GUIDE.default}\n\n💡 Get free soil test at your KVK — precise dosing saves ₹3,000+/acre.`;
    if (ql.includes("water") || ql.includes("irrigat") || ql.includes("sinch") || ql.includes("paani")) return `💧 Irrigation guide for ${stateCrop} in ${state}:\n\n${IRRIGATE_GUIDE[stateCrop]||IRRIGATE_GUIDE.default}`;
    if (ql.includes("price") || ql.includes("sell") || ql.includes("mandi") || ql.includes("market") || ql.includes("bhav") || ql.includes("bech")) {
      const cropRate = rates?.find(r => r.crop.toLowerCase() === stateCrop.toLowerCase());
      if (cropRate) return `💰 ${stateCrop} market update for ${state}:\n\nCurrent: ₹${cropRate.price.toLocaleString()}/qtl (${cropRate.change>=0?"+":""}${cropRate.change}% today)${cropRate.msp?`\nMSP: ₹${cropRate.msp}/qtl — ${cropRate.price >= cropRate.msp ? "✅ Price above MSP — good time to sell!" : "⚠️ Price below MSP — sell through govt procurement or wait."}`:""}\n\nBest price today across all crops: ${bestRate?.crop} at ₹${bestRate?.price?.toLocaleString()}/qtl (+${bestRate?.change?.toFixed(1)}%)\n\n💡 Register on e-NAM (enam.gov.in) for better prices across mandis.`;
      return `💰 Market advice for ${state}: Check our Market page for live prices. Register on e-NAM for access to 1,000+ mandis. Never sell below MSP — contact Kisan CC 1800-180-1551 if buyers offer below MSP.`;
    }
    if (ql.includes("loan") || ql.includes("kcc") || ql.includes("kisan credit") || ql.includes("finance") || ql.includes("money")) return `🏛️ Farmer loan options:\n\n• Kisan Credit Card (KCC): up to ₹3 lakh at 4% interest (2% after subsidy). Apply at any bank with land records.\n• PM-KISAN: ₹6,000/year direct bank transfer — register at pmkisan.gov.in\n• PM Fasal Bima Yojana: crop insurance at 1.5% premium (Rabi), 2% (Kharif)\n\nFor KCC: visit nearest bank branch with Aadhaar + land records. Approval in 7 days.`;
    if (ql.includes("soil") || ql.includes("ph") || ql.includes("mitti") || ql.includes("bhumi")) return `🪨 ${stateSoil} soil management for ${state}:\n\nTarget pH: 6.0–7.5 for most crops. Current typical pH for ${stateSoil}: ${stateSoil.includes("Black")||stateSoil.includes("Cotton")?"7.5–8.5 (alkaline — add gypsum 200kg/acre)":stateSoil.includes("Laterite")||stateSoil.includes("Red")?"5.0–6.5 (acidic — add lime 200kg/acre)":"6.0–7.0 (good range)"}\n\nOrganic matter: add FYM 4 tonnes/acre every season.\n\n🆓 Free soil health card at your KVK — test every 2 years.`;
    if (ql.includes("yield") || ql.includes("production") || ql.includes("output") || ql.includes("improve") || ql.includes("increase")) return `📈 Yield improvement for ${stateCrop} in ${state} (${stateSoil}):\n\n1. Use certified seeds from NSC/State Seed Corp\n2. Seed treatment: Thiram 3g/kg + Imidacloprid 5ml/kg\n3. ${FERT_GUIDE[stateCrop]?.split(".")[0]||"Apply balanced NPK"}\n4. Drip/sprinkler irrigation saves water + improves yield 20–30%\n5. Integrated Pest Management — early monitoring\n\n📞 KVK Demo Farm visit: 1800-180-1551`;
    if (ql.includes("weather") || ql.includes("rain") || ql.includes("mausam")) return `🌤️ ${state} weather right now: ${sd.temp}°C, ${sd.condition}, Humidity ${sd.hum}%.\n\n${sd.hum > 75 ? `⚠️ High humidity — watch for fungal diseases in ${stateCrop}. Avoid spraying in next 24h.` : sd.hum < 45 ? `⚠️ Low humidity + dry conditions — irrigate ${stateCrop} within 48 hours.` : `✅ Good conditions for field work and spraying.`}\n\nCheck our Weather page for 7-day forecast with farming alerts.`;
    if (ql.includes("scheme") || ql.includes("subsidy") || ql.includes("government") || ql.includes("yojana") || ql.includes("sarkar")) return `🏛️ Key government schemes for ${state} farmers:\n\n• PM-KISAN: ₹6,000/year — register at pmkisan.gov.in\n• PM Fasal Bima: crop insurance at 1.5–2% premium\n• Soil Health Card: free soil testing at KVK\n• e-NAM: online mandi, 1,000+ mandis, better prices\n• PM Krishi Sinchai Yojana: drip/sprinkler subsidy up to 55%\n• Kisan Credit Card: crop loans at 4% interest\n\n📞 Kisan Call Centre: 1800-180-1551 (free, Mon–Sat 6AM–10PM)`;
    // Default: comprehensive general advice
    return `🌾 Advice for ${stateCrop} farming in ${state} (${stateSoil} soil, ${sd.temp}°C):\n\n${alertAns ? `⚠️ Field Alert: ${alertAns}\n\n` : ""}Key actions this season:\n1. 🌱 ${FERT_GUIDE[stateCrop]?.split(".")[0]||"Apply balanced NPK at sowing"}\n2. 💧 ${IRRIGATE_GUIDE[stateCrop]?.split(".")[0]||"Irrigate at critical stages"}\n3. 🐛 Watch: ${PEST_GUIDE[stateCrop]?.split(".")[0]||"early pest scouting"}\n4. 💰 Current ${stateCrop} price: ${rates?.find(r=>r.crop.toLowerCase()===stateCrop.toLowerCase())?"₹"+(rates.find(r=>r.crop.toLowerCase()===stateCrop.toLowerCase())?.price?.toLocaleString())+"/qtl":"check Market page"}\n\n📞 KVK helpline: 1800-180-1551 (free)`;
  };
  ...
  const send = async (txt) => {
    ...
    try {
      ...
      setMsgs(m => {
        const updated = [...m, { role:"ai", text }];
        if (ttsEnabled) setTimeout(() => { ttsSpeak(text, lang); }, 200);
        return updated;
      });
    } catch(err) {
      // Smart offline fallback — never shows a raw error to the farmer
      const ans = getOfflineAnswer(q);
      setMsgs(m => [...m, { role:"ai", text: ans, offline: true }]);
  ...
  return (
    <div>
      <PageH icon="🤖" title={t.nav[10]} sub={`Personalised advisor for ${state||"your farm"} · ${stateCrop} · ${stateSoil}`}/>
      ...
      <div style={{ background:"linear-gradient(135deg,#1e1b4b,#312e81)", borderRadius:14, padding:"12px 18px", marginBottom:16, display:"flex", gap:12, alignItems:"center", flexWrap:"wrap" }}>
        ...
        <div style={{ flex:1 }}>
          <div style={{ color:"#c7d2fe", fontWeight:700, fontSize:13, marginBottom:2 }}>Powered by Claude AI — Knows Your Farm</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 290px", gap:16, height:"calc(100vh - 320px)", minHeight:450 }}>
        ...
        <div style={{ ...card, display:"flex", flexDirection:"column", padding:0, overflow:"hidden" }}>
          ...
          <div style={{ padding:"10px 14px", borderBottom:"1px solid #e5e7eb", display:"flex", justifyContent:"space-between", alignItems:"center", background:"#fafafa" }}>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              ...
              <span style={{ fontSize:12, fontWeight:600, color:"#374151" }}>AI Farm Advisor · {state||"India"}</span>
              ...
              <span style={{ fontSize:10, background:"#dbeafe", color:"#1e40af", borderRadius:10, padding:"1px 7px", fontWeight:600 }}>📚 Offline-ready</span>
            </div>
            <div style={{ display:"flex", gap:6 }}>
          ...
          <div style={{ borderTop:"1px solid #e5e7eb", padding:10, display:"flex", gap:8, alignItems:"center" }}>
            ...
            <MicBtn onResult={(text) => setInputVal(text)} lang={lang} />
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={card}>
            <div style={{ ...titleFont, fontWeight:700, fontSize:13, color:"#374151", marginBottom:10 }}>
              💬 {t.quickQ}
              <div style={{ fontSize:10, color:"#9ca3af", fontWeight:400, marginTop:1 }}>Tailored for {state||"your region"}</div>
            </div>
  ...
  useEffect(() => {
    ...
    const loadFirebase = async () => {
      try {
        ...
        // Public demo project — read/write open for hackathon demo
        const firebaseConfig = {
          apiKey: "AIzaSyDemo_KhetiSmart_Hackathon_2025",
          authDomain: "khetismart-demo.firebaseapp.com",
          projectId: "khetismart-demo",
          storageBucket: "khetismart-demo.appspot.com",
          appId: "1:000000000000:web:khetismart",
        };
        if (!window.firebase.apps.length) {
          window.firebase.initializeApp(firebaseConfig);
        }
        dbRef.current = window.firebase.firestore();
        collRef.current = dbRef.current.collection("listings");
        // Real-time listener filtered by state
        unsubRef.current = collRef.current
          .where("state", "==", state)
          .orderBy("createdAt", "desc")
          .limit(50)
          .onSnapshot(snap => {
            setListings(snap.docs.map(d => ({ id: d.id, ...d.data() })));
          }, err => setFbError("offline"));
        setFbReady(true);
      } catch(e) {
        setFbError("offline");
        // Graceful fallback to local state (no crash)
      }
    };
    loadFirebase();
...
function Marketplace({ t, rates, state, priceSource }) {
  ...
  const scoredBuyers = BUYERS.map(b => {
    let score = 0;
  ...
  return (
    <div>
      ...
      <div className="mob-full" style={{ display:"grid", gridTemplateColumns:"1fr 300px", gap:16 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={card}>
            ...
            <div style={{ marginBottom:8 }}>
              <input type="text" placeholder="Crop Name" value={listing.crop}
            ...
            {/* Qty */}
            <div style={{ marginBottom:8 }}>
              <input type="number" placeholder="Quantity (qtl)" value={listing.qty}
            ...
            <div style={{ marginBottom:8, position:"relative" }}>
              <input type="number" placeholder="Your Price (₹/qtl)" value={listing.price}
...
function SupplyMap({ t, selected, onSelect, onNavigate }) {
  ...
  const getBuyersForCrop = crop => {
    ...
    return BUYERS.filter(b => names.includes(b.name));
  };
  ...
  return (
    <div>
      <PageH icon="📊" title="Supply Intelligence Dashboard"
      ...
      <div className="mob-full" style={{ display:"grid", gridTemplateColumns:"1fr 296px", gap:14 }}>
        {/* Main table */}
        <div style={card}>
          ...
          <div style={{ display:"flex", gap:6, marginBottom:12, flexWrap:"wrap", alignItems:"center" }}>
            ...
            {[["All",t.supOpps||"Opportunity"],["Opportunity",t.supOpps||"Opportunity"],["Oversupply",t.supOvers||"Oversupplied"],["Watch",t.sigWatch||"Watch"]].map(([f,fl])=>(
            ...
            <span style={{ fontSize:11, fontWeight:700, color:"#374151", marginLeft:4 }}>{t.soilFilterLbl||"Soil"}:</span>
            ...
            <div style={{ marginLeft:"auto", display:"flex", gap:5, alignItems:"center" }}>
              <span style={{ fontSize:11, color:"#6b7280" }}>{t.sortFilterLbl||"Sort"}:</span>
              {[["supply","Supply%"],["trend","Trend"],["price","Price"]].map(([k,l])=>(
          ...
          <div style={{ display:"grid", gridTemplateColumns:"20px 1fr 72px 60px 62px 88px 82px",
            ...
            <div>#</div><div>{t.stateCropLbl||"State · Crop"}</div><div>{t.zoneLabel||"Zone"}</div>
            <div style={{textAlign:"center"}}>{t.supplyCol||"Supply"}</div>
            <div style={{textAlign:"center"}}>{t.trendCol||"Trend"}</div>
            <div style={{textAlign:"center"}}>{t.priceCol||"Price"}</div>
            <div style={{textAlign:"center"}}>{t.signalCol||"Signal"}</div>
          </div>
              ...
              <div key={r.region}>
                ...
                {isExp && (
                  <div style={{ margin:"0 0 8px 26px", padding:12, background:"#f9fafb",
                    ...
                    <div>
                      <div style={{ fontSize:11, fontWeight:700, color:"#374151", marginBottom:6 }}>📞 Buyers for {r.crop}:</div>
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        {rowBuyers.map(b=>(
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:13, color:"#166534", marginBottom:8 }}>🟢 Top Opportunities</div>
          ...
          <div style={card}>
            <div style={{ fontWeight:700, fontSize:13, color:"#991b1b", marginBottom:8 }}>🔴 Oversupplied — Avoid</div>
...
function CropSwitchAdvisor({ t, state, onSelect }) {
  ...
  return (
    <div>
      <PageH icon="🔄" title="Crop Switch Advisor"
        sub="Enter your state and current crop — get ranked alternatives with profit comparison and buyer contacts"/>

      <div className="mob-full" style={{ display:"grid", gridTemplateColumns:"360px 1fr", gap:16 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ ...card, background:"linear-gradient(135deg,#f0fdf4,#dcfce7)", border:"2px solid #86efac" }}>
            ...
            <div style={{ fontSize:12, color:"#374151", marginBottom:16, lineHeight:1.6 }}>
              We'll analyse national supply data and show you exactly which crops to switch to,
              how much more profit you can earn per acre, and which buyers are ready to purchase.
            </div>
            ...
            <label style={{ fontSize:12, fontWeight:700, color:"#374151", display:"block", marginBottom:4 }}>{t.currentCropLbl||"Current Crop (growing now)"}</label>
            ...
            {currentSR && (
              <div style={{ background:"rgba(255,255,255,0.8)", borderRadius:10, padding:"10px 12px", marginBottom:14 }}>
                <div style={{ fontSize:11, fontWeight:700, color:"#374151", marginBottom:6 }}>{t.currentSitLbl||"Current situation"}</div>
            ...
            <button onClick={runAdvisor} disabled={advLoading}
              ...
              {advLoading?"⏳ Analysing market data...":"🔍 Find Best Alternatives"}
            </button>
          </div>
        ...
        <div>
          {!advisorResult && !advLoading && (
            <div style={{ ...card, display:"flex", flexDirection:"column", alignItems:"center",
              ...
              <div style={{ fontSize:18, fontWeight:800, color:"#374151" }}>{t.readySwitchLbl||"Ready to find your best crop switch"}</div>
          ...
          {advLoading && (
            <div style={{ ...card, display:"flex", flexDirection:"column", alignItems:"center",
              ...
              <div style={{ fontSize:16, fontWeight:700, color:"#374151" }}>Analysing supply data for {advisorState}...</div>
          ...
          {advisorResult && !advLoading && (
            <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
              ...
              <div style={{ background:"linear-gradient(135deg,#1e40af,#1d4ed8)", borderRadius:14,
                ...
                <div style={{ fontSize:10, opacity:0.8, marginBottom:2, letterSpacing:1, textTransform:"uppercase" }}>{t.threeSourceLbl||"Three-Source Crop Switch Analysis"}</div>
                ...
                <div style={{ display:"flex", gap:24, flexWrap:"wrap" }}>
                  {[
                    ["Current supply", advisorResult.currentSupply+"%", advisorResult.currentSupply>70?"#fca5a5":"#86efac"],
                    ["Current net/acre", advisorResult.currentProfit?"₹"+advisorResult.currentProfit.net.toLocaleString():"—", "#fff"],
                    ["Best gain available", advisorResult.alternatives[0]?.gain>0?"+₹"+advisorResult.alternatives[0].gain.toLocaleString():"See below", "#86efac"],
                    ["Alternatives found", advisorResult.alternatives.length+" crops", "#fff"],
                  ].map(([k,v,vc])=>(
                    <div key={k}>
              ...
              <div style={{ fontWeight:700, fontSize:14, color:"#374151" }}>
                Recommended alternatives — ranked by additional profit per acre:
              </div>
                      ...
                      <div>
                        <div style={{ fontSize:17, fontWeight:800, color:"#111827" }}>
                          ...
                          {idx===0 && <span style={{ marginLeft:8, fontSize:11, background:"#f0fdf4",
                            color:"#166534", padding:"2px 8px", borderRadius:10, fontWeight:700 }}>{t.bestChoiceLbl||"Best Choice"}</span>}
                        </div>
                        <div style={{ fontSize:11, color:"#6b7280", marginTop:2 }}>
                  ...
                  {(() => {
                    ...
                    return (
                      <div style={{ background:"#eff6ff", borderRadius:8, padding:"7px 10px", marginBottom:10, fontSize:11, color:"#1e40af" }}>
                        🛡️ <strong>MSP Protection:</strong> ₹{msp.toLocaleString()}/qtl — government guarantees this minimum. If traders offer less, sell to state procurement agency.
                      </div>
                    );
                  })()}
...
function CropRec({ t, state }) {
  ...
  return (
    <div>
      <PageH icon="🌾" title={t.nav[2]} sub="Enter your farm details for personalised crop recommendations"/>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        ...
        <div style={card}>
          ...
          {results.length === 0 ? (
            <div style={{ textAlign:"center", padding:30, color:"#9ca3af" }}>No suitable crops found for this combination. Try adjusting water or season.</div>
          ) : results.map((c,i)=>{
    ...
    <div style={{ padding:"16px", maxWidth:900, margin:"0 auto" }}>
      ...
      <div style={{ background:"#f0fdf4", border:"2px solid #86efac", borderRadius:14, padding:"14px 18px", marginBottom:20 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12, flexWrap:"wrap", gap:8 }}>
          <div>
            <div style={{ fontWeight:800, color:"#14532d", fontSize:13 }}>🖨️ Krishi Mitra Handout Card</div>
            <div style={{ fontSize:11, color:"#6b7280", marginTop:2 }}>Print this. Hand it to farmers. No literacy needed to use the IVR number.</div>
          </div>
          <button onClick={() => window.print()} style={{ padding:"8px 16px", background:"#14532d", color:"#fff", border:"none", borderRadius:10, fontWeight:800, fontSize:12, cursor:"pointer", fontFamily:"inherit" }}>
            🖨️ Print Card
          </button>
        </div>
        {/* The card itself — designed to be printed */}
        <div style={{ background:"#fff", border:"3px solid #14532d", borderRadius:16, padding:"20px", maxWidth:380, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:16 }}>
            <div style={{ fontSize:36 }}>🌾</div>
            <div style={{ fontWeight:900, fontSize:18, color:"#14532d" }}>KhetiSmart</div>
            <div style={{ fontSize:12, color:"#6b7280" }}>{t.farmerHelpline||"Farmer Helpline — Free Call"}</div>
          </div>
          <div style={{ background:"#14532d", borderRadius:12, padding:"16px", textAlign:"center", marginBottom:14 }}>
            <div style={{ color:"#86efac", fontSize:12, fontWeight:700, marginBottom:4 }}>📞 Call this number:</div>
            <div style={{ color:"#fff", fontWeight:900, fontSize:28, letterSpacing:2 }}>1800-180-1551</div>
            <div style={{ color:"#86efac", fontSize:11, marginTop:4 }}>{t.freeMonSat||"Free · Mon–Sat · 6AM–10PM"}</div>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:14 }}>
            {[
              { icon:"1️⃣", label:"Press 1", desc:"Crop Prices" },
              { icon:"2️⃣", label:"Press 2", desc:"Crop Advice" },
              { icon:"3️⃣", label:"Press 3", desc:"Weather" },
              { icon:"5️⃣", label:"Press 5", desc:"Helpline" },
            ].map(({icon,label,desc})=>(
              <div key={label} style={{ background:"#f0fdf4", borderRadius:10, padding:"10px", textAlign:"center", border:"1px solid #bbf7d0" }}>
                <div style={{ fontSize:22 }}>{icon}</div>
                <div style={{ fontWeight:800, fontSize:12, color:"#14532d" }}>{label}</div>
                <div style={{ fontSize:10, color:"#6b7280" }}>{desc}</div>
              </div>
            ))}
          </div>
          <div style={{ background:"#fef9c3", borderRadius:10, padding:"10px 12px", textAlign:"center", fontSize:11, color:"#92400e", fontWeight:700 }}>
            ⚠️ If price is below MSP — Do NOT sell. Call helpline first.
          </div>
        </div>
      </div>
...
function CropInsuranceAdvisor({ t, state, rates }) {
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto" }}>
      <PageH icon="🛡️" title="Crop Insurance Advisor" sub="PM Fasal Bima Yojana (PMFBY) — Never lose your entire harvest income again"/>
      ...
      <div style={{ background:"linear-gradient(135deg,#1e3a5f,#1e40af)", borderRadius:18, padding:"20px 24px", marginBottom:20, display:"flex", gap:20, alignItems:"center", flexWrap:"wrap" }}>
        <div style={{ flex:1, minWidth:200 }}>
          <div style={{ color:"#93c5fd", fontSize:11, fontWeight:700, letterSpacing:1, marginBottom:8 }}>{t.whyMattersLbl||"WHY THIS MATTERS"}</div>
        ...
        <div style={{ background:"rgba(255,255,255,0.1)", borderRadius:14, padding:"16px 20px", textAlign:"center", minWidth:160 }}>
          <div style={{ color:"#93c5fd", fontSize:10, fontWeight:700, marginBottom:4 }}>COVERAGE PROTECTION</div>
      ...
      {tab==="overview" && (
        <>
          ...
          <div style={card}>
            ...
            {[
              { n:1, icon:"🏦", title:"Go to your KCC Bank or CSC", desc:`Visit your Kisan Credit Card bank, cooperative bank, or nearest Common Service Centre (CSC) before the deadline (${crop.window} — may vary by state). Tell them: "PMFBY enrolment chahiye for ${crop.name}."`, action:null },
              ...
              { n:3, icon:"✅", title:(t.payShareOnly||"Pay your share only"), desc:`Pay ₹${Math.round(farmerShare)} (your ${farmerPremiumPct}% of Sum Insured — ${crop?.season} rate). Bank processes the rest with government subsidy. You'll get an SMS confirmation with your policy number.`, action:"tel:18001801551" },
            ].map(s=>(
      ...
      {tab==="calculator" && (
        <div style={card}>
          ...
          <div style={{ marginBottom:14 }}>
            <label style={{ fontWeight:600, fontSize:13, color:"#374151", display:"block", marginBottom:6 }}>Farm size: <strong style={{color:"#14532d"}}>{acres} acres</strong></label>
...
const NAV_IDS   = ["dashboard","weather","crop","switchadvisor","supplymap","highvalue","soil","profit","market","marketplace","ai","rural","insurance"];
...
const DEMO_STEPS = [
  { page:"dashboard",   icon:"🏠", title:"Meet Ramesh", sub:"Wheat farmer · Punjab · 3 acres",
    highlight:"This is Ramesh's dashboard. In one glance he sees today's weather, his soil status, live wheat prices, and his top recommendation — all personalised to Punjab." },
  { page:"weather",     icon:"🌤️", title:"Check the forecast",  sub:"Plan irrigation & spraying",
    highlight:"Live weather from Open-Meteo. Ramesh can see a 7-day forecast and farming alerts — 'Avoid spraying Thursday, rain expected'. No more guessing." },
  { page:"soil",        icon:"🌱", title:"IoT Soil Monitor",    sub:"Live field sensor readings",
    highlight:"4 field sensors show moisture, pH, nitrogen and potassium in real time. Red alert on Plot B — nitrogen is low. Ramesh knows to apply urea today." },
  { page:"ai",          icon:"🤖", title:"Ask the AI Advisor",  sub:"In Hindi, English, or Punjabi",
    highlight:"Ramesh types 'मेरी गेहूं में कौन सी बीमारी आ सकती है?' and gets a specific, personalised answer in Hindi with exact doses — even offline." },
  { page:"market",      icon:"📊", title:"Live Mandi Prices",   sub:"Never sell below MSP",
    highlight:"Wheat is ₹2,350/qtl — above MSP of ₹2,275. The app says 'Good time to sell.' No more relying on middlemen for price info." },
  { page:"marketplace", icon:"🏪", title:"Sell Directly",       sub:"Skip the middlemen",
    highlight:"Ramesh lists his 20 qtl of wheat. Three verified buyers get notified. He negotiates directly — no commission, no market fee." },
  { page:"rural",       icon:"📡", title:"Works on Any Phone",  sub:"800M feature phone users",
    highlight:"No smartphone? No problem. *121# USSD gives the same price data and crop advice on any ₹500 keypad phone, zero internet needed." },
];
...
export default function App() {
  ...
  const renderPage = () => {
    switch(page) {
      ...
      case "supplymap":   return <SupplyMap t={t} selected={state} onSelect={s=>setState(s)} onNavigate={setPage}/>;
      ...
      <div style={{ display:"flex", height:"100vh", background:"linear-gradient(160deg, #f0f7f2 0%, #f8fafc 60%, #f0f4f8 100%)", fontFamily:"'DM Sans', system-ui, sans-serif", overflow:"hidden" }}>
        ...
        <aside className="desktop-sidebar" style={{ width:collapsed?62:224, background:"linear-gradient(180deg,#0f3d1f 0%,#14532d 35%,#166534 70%,#1a6b3a 100%)", flexDirection:"column", transition:"width 0.3s cubic-bezier(0.4,0,0.2,1)", overflow:"hidden", flexShrink:0, boxShadow:"2px 0 16px rgba(0,0,0,0.15)" }}>
          ...
          <nav style={{ flex:1, padding:"8px 6px", overflowY:"auto", overflowX:"hidden" }}>
            {NAV_IDS.map((id,i)=>(
              <button key={id} onClick={()=>setPage(id)} style={{ display:"flex", alignItems:"center", gap:9, width:"100%", padding:"9px 8px", borderRadius:9, border:"none", background:page===id?"rgba(255,255,255,0.15)":"transparent", color:page===id?"#fff":"#86efac", cursor:"pointer", textAlign:"left", marginBottom:1, fontSize:13.5, fontWeight:page===id?600:400, fontFamily:"inherit", transition:"all 0.2s" }}>
                <span style={{ fontSize:16, flexShrink:0 }}>{NAV_ICONS[i]}</span>
                {!collapsed && <span style={{ whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{t.nav[i]}</span>}
                {page===id && !collapsed && <span style={{ marginLeft:"auto", width:5, height:5, background:"#fff", borderRadius:"50%", flexShrink:0 }}/>}
              </button>
            ))}
          </nav>
          {/* Helpline */}
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden" }}>
          ...
          <header style={{ background:"rgba(255,255,255,0.92)", backdropFilter:"blur(12px)", borderBottom:"1px solid rgba(0,0,0,0.06)", padding:"8px 16px", flexShrink:0, boxShadow:"0 1px 8px rgba(0,0,0,0.04)" }}>
            {/* Row 1: page title + live badge + avatar */}
        ...
        <nav className="mob-bottom-nav" style={{
          ...
          {[
            ...
            { id:"weather",   icon:"🌤️", label:t.nav[1] },
            { id:"market",    icon:"📊", label:t.nav[8] },
          ...
          <div ref={moreNavRef} style={{ position:"relative" }}>
            ...
            {showMoreNav && (
              <div style={{ position:"absolute", bottom:60, right:0, background:"#14532d", borderRadius:12, padding:8, boxShadow:"0 -4px 24px rgba(0,0,0,0.4)", zIndex:600, minWidth:160, border:"1px solid rgba(255,255,255,0.15)" }}>
                {[
                  { id:"crop",         icon:"🌾", label:t.nav[2] },
                  { id:"switchadvisor",icon:"🔄", label:t.nav[3] },
                  { id:"supplymap",    icon:"🗺️", label:t.nav[4] },
                  { id:"highvalue",    icon:"💎", label:t.nav[5] },
                  { id:"profit",       icon:"💰", label:t.nav[7] },
                  { id:"marketplace",  icon:"🏪", label:t.nav[9] },
                  { id:"rural",        icon:"📡", label:t.nav[11] },
                  { id:"insurance",    icon:"🛡️", label:t.nav[12]||t.nav[11] },
                ].map(item=>(
